/* Online play for Shogi via PeerJS (WebRTC).
 * Host creates a 5-digit room and plays 先手(Sente); guest plays 後手(Gote).
 * Full move objects are relayed peer-to-peer.
 */
(function () {
  const Game = window.ShogiGame;

  const ALPHABET = "0123456789";  // 5-digit room codes
  const PREFIX = "shogi-honsho-"; // namespace per game
  const HOST = 0, GUEST = 1;      // Shogi: SENTE=0 (host), GOTE=1 (guest)
